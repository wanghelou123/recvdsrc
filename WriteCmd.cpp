...
cout << __FILE__<< "	" << __LINE__  << "	" << __FUNCTION__ <<endl;
		...
		if(-1 == db.ConnectDB(dbaddr, dbport, dbname, dbuser, dbpwd, 60)){
			log_output("%s %s connect DB failed!\n",__FILE__, __FUNCTION__ );
			return ;		
		}
		...
		if(0 == i){
			log_output("No Command require write");
			db.DBclear(res);
		...
		for(s=0; s<i;s++){
				...
				if(theIterator == gateway_object_list.end()){//没有该网关的配置
					...
					#ifdef DEBUG
						cout<<__FUNCTION__<<"cant not find the gateway:"<< gateway_id<<endl;
					#endif
					//return;
					continue;//modified by whl 2013-7-30
				}else{
				#ifdef DEBUG	
					cout << "find the gateway:"<< gateway_id << endl; 
				#endif	
				...
				if(atoi(db.DBgetvalue(res,s,5))==2){
						...
						if(i_sec >= 20){
							...
							log_output(" WriteCmd::GetCmdFromDB()  timeout!\n");
							db.DBclear(res);
							db.DisConnectDB();
				...
				if(0 == p_gateway->write_flag){
						...
						if(p_gateway->timer_flag == 1){
							...
							printf("%d:%s:%s:i_sec=%d\n",__LINE__, __FILE__, __FUNCTION__, (int)p_gateway->i_sec);
						}
...
void   WriteCmd::correctTime( )
	...
	if(0 == i){
		cout <<" No Command require write"<<endl;
		db.DBclear(res);
		db.DisConnectDB();//断开数据库的连接，直接返回不再注册时器了
		return ;	
		}
