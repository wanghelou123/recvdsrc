...
bool handle_msg(GatewayDB &db, tcp_message_ptr& p, bool is_delete)
{
	...
	for(i=0,j=9;i<32 && j<p->size;i++,j+=4){//解析每路数据
		...
		snprintf(sql1, sizeof(sql1) - 1, \
			"INSERT INTO data \
			(data_time, gateway_logo, sensor_name, channel_name, value) values ");
		...
		try{
			...
			snprintf(tmp, sizeof(tmp) - 1, \
				"'%s','%s', %d, %d, %d, %.3f),", \
				strTime, p->gateway_id,  p->data[6], channelNum, dataName, dataValue);
#else
			snprintf(tmp, sizeof(tmp) - 1, \
			...
			snprintf(tmp, sizeof(tmp) - 1, \
				"'%s','%s', %d, %d, %.3f),", \
				strTime, p->gateway_id,  p->data[6], dataName, dataValue);
#else
			snprintf(tmp, sizeof(tmp) - 1, \
				"(to_timestamp('%s', 'YYYY-MM-DD HH24:MI:SS'), \
				'%s', %d, %d, %.3f),", \
